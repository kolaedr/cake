    ...
    {!! Form::label('image_label', 'Cake Filling image:') !!}
      <div class="input-group">
        <input type="text" id="image_label" class="form-control" name="image"
               aria-label="Image" aria-describedby="button-image" value="{{$AdditionalDecorations->image??''}}">
        <div class="input-group-append">
            <button class="btn btn-outline-secondary" type="button" id="button-image">Select</button>
        </div>
    </div>
</div>
@if ($AdditionalDecorations->image)
   <div class="form-group">
        <label for="">Image</label>
        <img src="{{$AdditionalDecorations->image}}" alt="" class="thumbnail admin-edit-image" style="width:50em">
